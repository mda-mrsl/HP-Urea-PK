...
% VIF known accurately and fixed in fitting
%   Runtime is ~5mins on laptop with i7-10610U processor
clear variables
close all

rng(4445, 'twister'); % for reproducibility
...
for ii = 1:nIter  % SNR vals
    ...
    for kk=1:nReps
        ...
        % 3 params, full model
        residMx   = gMx1 - repmat(reshape(noisyMx, 1, 1, 1, []), ...
                                 [gSize1, 1]);
        ...
        % 2-params, (kve, ve), vb = 0
        residMx   = gMx2 - repmat(reshape(noisyMx, 1, 1, []), ...
                                 [gSize2, 1]);
        ...
        residMx   = gMx3 - repmat(reshape(noisyMx, 1, 1, []), ...
                                 [gSize3, 1]);
        ssrMx     = sum(residMx.^2, 3);
        [~, idx]  = min(ssrMx(:));
