function [modelParams, resnorm, residual, exitflag, output] = ...
            fitPerfModel2(Mx, fitParams, acqParams, fixParams, options)
...
%#          vsc - Signal scaling factor (same as VIF amplitude scaling)
%#            Defaults: guess = 1,   lower = 0.5,   upper = 1.5
%#
%#  OPTIONAL INPUTS:
...
defaultFitParams = struct( ...
    ...
    'vsc', struct('guess', 1,    'lower', 0.5,  'upper', 1.5));
...
for field = 1:numel(fitFields)               % vb, vei, kve, T1 and/or vsc
    curField = fitFields{field};
...
vsc             = modelParams.vsc;
modelParams     = rmfield(modelParams, 'vsc');
Mx = vsc .* runPerfModel(acqParams, modelParams);
