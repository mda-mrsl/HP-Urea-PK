function [modelParams, resnorm, residual, exitflag, output] = ...
            fitPerfModelVif(Mx, fitParams, acqParams, fixParams, options)
...
%#  Unlike fitPerfModel, this function can also fit the scale, shape and  
%#  timing of a gamma-variate vascular input function.
%#
%#  REQUIRED INPUTS:
...
%#          vifMax   - Max amplitude of gamma-variate VIF
%#            Defaults: guess = 100,  lower = 0,    upper = Inf
%#          vifShape - Shape factor of gamma-variate VIF
%#            Defaults: guess = 1.8,  lower = 0,    upper = Inf
%#          vifTMax  - Time of max gamma-variate VIF amplitude        [s]
%#            Defaults: guess = 8.1,  lower = 0,    upper = Inf
%#          vifDelay - Delay time to onset of gamma-variate VIF       [s]
%#            Defaults: guess = 0,    lower = -Inf, upper = Inf
%#
%#  OPTIONAL INPUTS:
...
%#      output      - Structure output from lsqcurvefit
%#
%#  LITERATURE:
...
defaultFitParams = struct( ...
    ...
    'vifMax',   struct('guess', 100, 'lower', 0,    'upper', Inf),...
    'vifShape', struct('guess', 1.8, 'lower', 0,    'upper', Inf),...
    'vifTMax',  struct('guess', 8.1, 'lower', 0,    'upper', Inf),... % [s]
    'vifDelay', struct('guess', 0,   'lower', -Inf, 'upper', Inf));   % [s]
...
for field = 1:numel(fitFields)               % vb, vei, kve, T1 and/or vif params
    curField = fitFields{field};
...
for field = 1:numel(fitFields)
    curField = fitFields{field};
    if regexp(curField, 'vif')
        modelParams.vif.(curField) = fitValues(field);
    else
